...
parser.add_argument(
    "--dir", type=str, default="ckpt", help="path to save checkpoints (default: None)"
)
parser.add_argument(
    "--data_path",
    type=str,
...
parser.add_argument(
    "--epochs", type=int, default=200, help="number of epochs to train (default: 10)"
)
parser.add_argument(
    "--batch-size",
    type=int,
...
def update_params(lr, epoch):
    for p in net.parameters():
        ...
        if (epoch % 50) + 1 > 45:
            ...
            buf_new += (
                2.0 * lr * args.alpha * args.temperature / datasize
            ) ** 0.5 * eps
        p.data.add_(buf_new)
        p.buf = buf_new
...
def test(epoch):
    ...
    print(
        "\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n".format(
            test_loss / len(testloader), correct, total, 100.0 * correct.item() / total
        )
    )
